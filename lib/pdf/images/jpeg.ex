defmodule Pdf.Images.JPEG do
  ...
  defp parse_jpeg(<<255, 216, rest::binary>>, io), do: parse_jpeg(rest, io)
  ...
  defp parse_jpeg(_data, _io), do: {:error, :not_jpeg}

  def parse_image_data(
        <<bits, height::unsigned-integer-size(16), width::unsigned-integer-size(16), channels,
          _rest::binary>>,
        _io
      ) do
    {:ok, {bits, height, width, channels}}
  end

  def parse_image_data(_, _), do: {:error, :parse_error}

  defp get_chunk(io), do: {:ok, IO.binread(io, @file_chunk_size), io}
  ...
  defp ensure_length(data, length, io) when byte_size(data) >= length, do: {:ok, data, io}
